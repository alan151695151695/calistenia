...
const META_PIXEL_ID = "1120879250017867";              // <- tu Pixel ID
const META_ACCESS_TOKEN = "EAAOv6tJB7hYBPMgtjZChULwcity7aU0WLMWGyzAOx14wlCYcZA9OAqplOJ5OAo092qHUpZBCIYRBhVm2XiwHRmZCyK6us2gOBFqQW6jGNDnJoZCjtqa2Qhx8mc9xMWy5D55Tm1PY5epx0IywFRq2ZAz0ehFJDEPiUnbsxsfptZAkTItc0HHMsKZAX7dlzWsv2ZAwyEAZDZD"; // <- token CAPI
...
export async function POST(req) {
  ...
  const status = (body?.status || body?.event?.status || body?.payment?.status || "")
    .toString()
    .toUpperCase();

  const amount   = body?.amount ?? body?.payment?.amount ?? body?.transaction?.amount ?? 0;
  ...
  if (status === "APPROVED") {
    await sendMetaPurchase({ value: amount, currency, orderId, eventSourceUrl });
  } else {
